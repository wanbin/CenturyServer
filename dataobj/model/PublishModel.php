...
class PublishModel extends BaseModel {
	...
	protected function getRandomOne($type){
		...
		$res=$this->oneSql("select user_publish.*,username,date(FROM_UNIXTIME(time)) sendtime from user_publish,wx_account where wx_account.gameuid=user_publish.gameuid and isshow=1  limit $index,1");
		return $res;
	}
